...
var initialPopulation = function(req, res, next){
	setDefaultHeader(res);
	...
	areaOne.locations = [];
	areaOne.locations.push(location0);
	...
	for (var i = 0; i < areaArray.length; i++) {
		...
		numLocations += areaArray[i].locations.length;
	};
	
	var responseString = "Database populated with " + areaArray.length + " areas and " + numLocations + " locations";
...
server.get("/dontcallmetwice/populate", initialPopulation);
